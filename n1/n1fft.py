...
def get_N1pyfftw_XY(npixs, fals, xory='x', ks='p', Laxis=0,
    ...
    box = get_ellmat(lminbox, lmaxbox)
    shape = box.shape
    fft_shape = box.rshape if rFFT else box.shape
    ...
    if fftw:
        def ifft(arr):
            ...
            outp = pyfftw.empty_aligned(box.shape, dtype='float64' if rFFT else 'complex128')
    ...
    vpix = (np.prod(box.lsides) / np.prod(box.shape))
    norm =  0.25 * vpix ** (-2) # overall normalization, using iDFT = Vpix wanted FT's and 1/2 factor in GMV est.
    Ft = np.zeros(max(box.ellmax + 1, len(fals['tt'])))
    Ft[:len(fals['tt'])] = fals['tt']

    print('lmin max %s %s ' % (int(2. * np.pi / box.lsides[0]), box.ellmax) + str(box.shape))
    ...
    ls = np.int_(2 * np.pi * np.sqrt(nx ** 2 + ny ** 2) / box.lsides[0])
    print(np.min(ls[np.where(ls > 0)]), np.max(ls))
    CTT = extcl(box, cls_grad['tt'])
    if cpp is None :
        cpp = cls_unl['pp'][:lmaxphi + 1]
    cpp[lmaxphi + 1:] *= 0.
    cpp[:lminphi] *= 0.
    XIPP = ifft(extcl(box,cpp)[ls])

    FX = FY = FI = FJ = Ft[ls]

    dL2did = lambda L: int(box.lsides[0] * L / (2 * np.pi))
    dnpix2L = lambda npix: 2. * np.pi / box.lsides[0] * npix
