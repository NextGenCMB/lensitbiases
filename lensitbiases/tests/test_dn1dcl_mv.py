...
# Testing N1_fid = 1/2 dn1/dcl cl  (1/2 because qudratic function)
pl.plot(Ls, n1)
n1_emp_tt = 0.5 * np.array([np.dot(dnmat[i, 0], cls_len['tt'][:lmax + 1]) for i in range(len(Ls))])
n1_emp_te = 0.5 * np.array([np.dot(dnmat[i, 1], cls_len['te'][:lmax + 1]) for i in range(len(Ls))])
n1_emp_ee = 0.5 * np.array([np.dot(dnmat[i, 2], cls_len['ee'][:lmax + 1]) for i in range(len(Ls))])
n1_emp_bb = 0.5 * np.array([np.dot(dnmat[i, 2], cls_len['bb'][:lmax + 1]) for i in range(len(Ls))])
