...
class nhl_fft:
    def __init__(self, cls_noise, cls_w, lminbox=50, lmaxbox=2500, lx_cut=0, k2l=None, cls_w2=None):
        """
        ...
        cls_noise = {k: extcl(self.box.lmaxbox + int(self.box.lminbox) + 1, cls_noise[k]) for k in cls_noise.keys()}  # filtered maps spectra
    ...
    def _build_K(self, i, j):   
        # Here cls_w1 is the same as cl_cmb
        ret = np.zeros(self.box.rshape, dtype=complex)
        cmb = self.cls_cmb[i, j][self.box.ls()]
        Ki = (cmb + self.cls_noise[i, j][self.box.lx_int()])
        ret[np.where(cmb != 0)] = 1. / Ki[np.where(cmb != 0)]
        ret *= (self.box.lx_int() >= self.lx_cut)
        return ret
