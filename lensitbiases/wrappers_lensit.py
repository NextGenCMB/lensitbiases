...
class cmbconf:
    def __init__(self, k, jt_TP, nlevt, nlevp, transf, lmin, lmax,
                 cls_cmblen=None, cls_cmbresponse=None, cls_cmbfilt=None,
                 cls_cmblen_sims=None, cls_qeweight=None, cls_cmblunl=None):
        ...
        if cls_cmblen_sims is None: # sims cmb power
            cls_cmblen_sims = cls_cmblen
        ...
        ivfs_cls_dd, fals = get_ivf_cls(cls_cmblen, cls_cmbfilt, self.lmin, self.lmax, self.nlevt, self.nlevp, self.nlevt, self.nlevp,
    ...
    def get_N0_4dsm2ss_iterative_2d(self, itermax, nmax, lminbox=14.179630807244129, lmaxbox=7258):
        assert self.k in ['p_p', 'p', 'ptt'], self.k
        assert itermax >= 0, itermax
        lmax_qlm = 2 * self.lmax
        N0s = []
        ...
        for irr, it in enumerate_progress(range(itermax + 1)):
            ...
            ivfs_cls_ss, fals = get_ivf_cls(self.cls_unl, cls_filt, self.lmin, self.lmax, self.nlevt, self.nlevp,  self.nlevt, self.nlevp, self.transf,
                                         jt_tp=self.jt_TP)
            ivfs_cls_dd, fals = get_ivf_cls(cls_plen, cls_filt, self.lmin, self.lmax, self.nlevt, self.nlevp,  self.nlevt, self.nlevp, self.transf,
                                         jt_tp=self.jt_TP)
            cls_f_2d = lib_len.lensed_gradcls_2d(nmax=nmax) # response cls
            cls_f = {k: lib_len.box.sum_in_l(cls_f_2d[k]) * cli(lib_len.box.mode_counts() * 1.) for k in cls_f_2d.keys()}
            # Always unlensed weights
            cls_w = {k:np.copy(self.cls_unl[k]) for k in self.cls_unl.keys()}
            if self.k == 'ptt':
                fals['ee'] *= 0.
                fals['bb'] *= 0.
                ivfs_cls_ss['ee'] *= 0.
            ...
            if self.k in ['ptt', 'p_p']:
                cls_w['te'] *= 0.
            nhllib = n0_fft.nhl_fft(ivfs_cls_ss, cls_w,  lminbox=lminbox, lmaxbox=lmaxbox, k2l=self.k2l)
            box = nhllib.box

            # NB: could possibly use 1d and spline to get all modes in the box
            ss = nhllib.get_nhl_ds_2d('p_p',  {k: n0_fft.extcl(box.lmaxbox + int(box.lminbox) + 1, ivfs_cls_ss[k]) for k in ivfs_cls_ss.keys()})[0]
            ds = nhllib.get_nhl_ds_2d('p_p',  {k: n0_fft.extcl(box.lmaxbox + int(box.lminbox) + 1, ivfs_cls_dd[k]) for k in ivfs_cls_dd.keys()})[0]

            ds = box.sum_in_l(ds) * cli(box.mode_counts() * 1.)
            ss = box.sum_in_l(ss) * cli(box.mode_counts() * 1.)

            r_gg, r_cc = n0_fft.nhl_fft(fals, cls_f,  lminbox=lminbox, lmaxbox=lmaxbox, k2l=self.k2l, cls_w2=cls_w).get_nhl_2d(self.k) # Could try to check when n = r
            r_gg =  lib_len.box.sum_in_l(r_gg)  * cli(lib_len.box.mode_counts() * 1.)

            N0 =  (4 * ds - 2 * ss) * cli(r_gg) ** 2
            N0s.append(N0[:lmax_qlm+1])
            N0pN1 = N0
            ls, = np.where(lib_len.box.mode_counts()[:lmax_qlm+1])
        if False:
    ...
    def get_n0(self, lminbox=14.179630807244129, lmaxbox=7258):
        lib = n0_fft.nhl_fft(self.ivfs_cls_dd, self.cls_w, lminbox=lminbox, lmaxbox=lmaxbox, k2l=self.k2l)
