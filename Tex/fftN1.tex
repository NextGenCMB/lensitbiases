...
\end{equation}
(There are two terms in this sum, one here the $C^{w}$ acts on $\bl_1$ and one where it acts on $\bl_2$). Each factor brings a $i\bl^a C^{f}_{\bl}$ in the relevant place (outer terms). The Cls involved hence are $C^f \cdot C^w  \cdot F$, $C^f \cdot F$, $F \cdot C^f$, $ C^w \cdot F \cdot C^f$, $F$ and $C^w F$. \color{red} should F and w be reversed in the second term? does not like symmtric in this form \color{black}
...
\begin{equation}
	...
	\end{pmatrix}_{XY}
\end{equation}
Since each $W_{\bll}^{ST}$ by construction is symmetric in $\bl_1 = \bl + \bll/2, \bl_2 = \bl - \bll /2$, we have the following symmetries:
\begin{equation}
	W_{\bll}^{ST, (0,0)} = W_{\bll}^{TS, (0,0)}
\end{equation}
